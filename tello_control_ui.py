...
class TelloUI:
    ...
    def openCmdWindow(self):
        ...
        text0 = tki.Label(panel,
                          text='This Controller map keyboard inputs to Tello control commands\n'
                               'Adjust the trackbar to set distance and degree parameter',
                          font='Helvetica 10 bold'
                          )
        text0.pack(side='top')

        textCus1 = tki.Label(panel,
                          text='Group Dunessh Tello Drone Project',
                          font='Helvetica 10 bold'
                          )
        textCus1.pack(side='top')
        ...
        self.btn_close = tki.Button(
            panel, text="Close program", relief="raised", command=self.onClose)
        self.btn_close.pack(side="bottom", fill="both",
                              expand="yes", padx=10, pady=5)

        self.btn_interrupt = tki.Button(
            panel, text="Interrupt and stop", relief="raised", command=self.interrupt)
        self.btn_interrupt.pack(side="bottom", fill="both",
                              expand="yes", padx=10, pady=5)

        self.btn_battery = tki.Button(
            panel, text="Check battery", relief="raised", command=self.checkBattery)
        self.btn_battery.pack(side="bottom", fill="both",
                              expand="yes", padx=10, pady=5)
        
        self.btn_flightTime = tki.Button(
            panel, text="Check flight time", relief="raised", command=self.checkFlightTime)
        self.btn_flightTime.pack(side="bottom", fill="both",
                              expand="yes", padx=10, pady=5)

        self.btn_height = tki.Button(
            panel, text="Check current height", relief="raised", command=self.checkHeight)
        self.btn_height.pack(side="bottom", fill="both",
                              expand="yes", padx=10, pady=5)

        self.btn_speed = tki.Button(
            panel, text="Check speed", relief="raised", command=self.checkSpeed)
        self.btn_speed.pack(side="bottom", fill="both",
                              expand="yes", padx=10, pady=5)

        self.btn_preplan = tki.Button(
            panel, text="Preplan route", relief="raised", command=self.telloPreplan)
        ...
        # self.tmp_f.bind('<KeyPress-Escape>',self.on_esc_press)
    ...
    def telloPreplan(self):
        return self.tello.preplan()
    
    def checkBattery(self):
        return self.tello.get_battery()

    def checkFlightTime(self):
    ...
    def interrupt(self):
        return self.tello.interrupt()

    def telloClose(self):
        return self.tello.close()
